...
> **Addendum (this pass):** the "Current status" section below is stale,
> auth linking and RLS were added later in `supabase/migrations/001` through `006`.
> A new `supabase/migrations/007_realtime_and_profile_fixes.sql` has been
> added on top of those and **must be run** (SQL Editor, in order after
> 006) before profile self-service, avatar uploads, maintenance mode, or
> any live/realtime updates will work. See the root-cause writeup for
> details on what it fixes and why.
...
module by module, testing each against your live database before moving on, not as a single all-at-once rewrite.
