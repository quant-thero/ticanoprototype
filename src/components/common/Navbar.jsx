...
const TYPE_ICON = { complaint: '', escalation: '', feedback: '', queue: '', message: '', alert: '', referral: '', report: '', campaign: '', system: '', lead: '👤', announcement: '📢' };
...
export default function Navbar({ title }) {
  ...
  const openNotification = (n) => {
    ...
    const base = ROLE_DASHBOARD[user?.role];
    if (!base) return;
    if (!n.tab) return navigate(base);

    // Complaint-specific notifications encode the id as 'complaints:123'
    // so clicking one opens that exact complaint, not just the general tab.
    const [tab, entityId] = n.tab.split(':');
    const params = new URLSearchParams({ tab });
    if (entityId) params.set('complaintId', entityId);
  ...
  return (
    <nav className="bg-white dark:bg-ticano-dark-card border-b border-gray-100 dark:border-gray-700/60 text-ticano-charcoal dark:text-white px-4 py-2 flex items-center justify-between shadow-sm sticky top-0 z-50 backdrop-blur-sm">
      <div className="flex items-center gap-3 min-w-0">
        <button onClick={goHome} className="flex items-center rounded-lg hover:opacity-80 transition-opacity focus:outline-none focus:ring-2 focus:ring-ticano-red shrink-0" aria-label="Go to dashboard">
          <Logo size={28} withTagline taglineClassName="text-gray-400 dark:text-gray-500 hidden sm:block" />
        </button>
        {title && (
          <>
