...
import {
  previewTenderRecipients, getTenderBroadcasts, createTenderBroadcast,
  deleteTenderBroadcast, getClientIndustries,
} from '../../services/api';
...
export default function TenderBroadcasts() {
  ...
  useEffect(() => { getClientIndustries().then(({ data }) => setIndustries(data || [])); }, []);
  useEffect(() => { previewTenderRecipients(filters).then(({ data }) => setPreview(data)); }, [filters]);
  const loadHistory = () => { getTenderBroadcasts(query).then(({ data }) => setHistory(data)); };
