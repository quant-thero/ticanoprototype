...
import {
  ...
  assignComplaint, reassignComplaint, reassignComplaintToNew, updateComplaintStatus, updateComplaintPriority,
  addInternalNote, addCustomerNote, updateSentiment,
  escalateComplaint, resolveComplaint, closeComplaint,
  recommendPm, getAuditTrail, subscribeToTable,
...
export default function ComplaintsModule({
  ...
  // Deep-link from a notification (e.g. "New update on TCN-0042") straight
  // to that complaint, instead of just landing on the general list.
  useEffect(() => {
    if (initialComplaintId) setSelectedId(Number(initialComplaintId));
  }, [initialComplaintId]);
  ...
  // Live sync: without this, an action on one person's screen (e.g. a PM
  // escalating a complaint to a Service Manager) only shows up for anyone
  // else once they manually refresh or re-navigate. This re-runs load()
  // whenever a complaint or escalation row changes anywhere in the system
  //, cheap and simple, and correct even for cross-branch/cross-role
  // visibility rules already encoded in load() above.
  useEffect(() => {
    const unsubComplaints = subscribeToTable('complaints', {}, load);
    const unsubEscalations = subscribeToTable('complaint_escalations', {}, load);
    return () => { unsubComplaints(); unsubEscalations(); };
  }, [scope, currentUser?.id, branchFilter]);

  useEffect(() => {
    if (!selectedId) { setOpen(null); return; }
    getComplaintById(selectedId).then(({ data }) => setOpen(data)).catch((err) => { console.error('[ComplaintsModule]', err); toast.error('Could not load complaint details'); setOpen(null); });
  ...
  if (selectedId && open) {
    return (
      <ComplaintDetail
        ...
        onPriorityChange={(p) => {
          updateComplaintPriority(open.id, p, currentUser?.name || 'Staff').then(() => {
            toast.success('Priority updated');
            load();
          }).catch((err) => toast.error(err?.message || 'Could not update priority'));
        }}
        AssignModalEl={
          <AssignModalContent
            isOpen={assignModal}
...
function ComplaintDetail({
  ...
  onAddInternal, onAddCustomer, onSentiment, onPriorityChange,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        {showInternalNotes && onPriorityChange ? (
          <div>
            <p className="text-[11px] uppercase tracking-wide text-gray-400 mb-1">Priority</p>
            <select
              value={c.priority}
              onChange={(e) => onPriorityChange(e.target.value)}
              className={`text-xs font-semibold px-2 py-1.5 rounded-lg border-0 cursor-pointer ${PRIORITY_TONE[c.priority] || ''}`}
            >
      ...
      <div className="bg-white dark:bg-ticano-dark-card rounded-xl border border-gray-100 dark:border-gray-700 p-5">
        ...
        <p className="text-gray-800 dark:text-gray-200 leading-relaxed">{c.description || <span className="text-gray-400 italic">No written description, see voice note below.</span>}</p>
        {c.hasVoiceNote && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
            <p className="text-xs uppercase tracking-wide text-gray-500 mb-2 flex items-center gap-1.5"><Mic size={12} className="text-ticano-red" /> Customer voice note</p>
            <VoiceNotePlayer voiceNotePath={c.voiceNotePath} duration={c.voiceNoteDuration} uploadedAt={c.voiceNoteUploadedAt} />
            <p className="text-[11px] text-gray-400 mt-1">Streamed securely within the portal. Downloading and exporting are disabled.</p>
          </div>
        )}
      </div>
      ...
      {!terminal && (
        <div className="flex flex-wrap gap-2">
          ...
          {canEscalate && c.escalation?.to !== (currentUser?.role === 'service_manager' ? 'director' : 'service_manager') && (
            <button onClick={onEscalate} className="px-4 py-2 rounded-lg border border-orange-400 text-orange-700 text-sm hover:bg-orange-50 flex items-center gap-1.5">
              <ArrowUpCircle size={14} /> {currentUser?.role === 'service_manager' ? 'Escalate to Director' : 'Escalate'}
            </button>
          )}
          ...
          {(c.status === 'resolved' || c.status === 'closed') && (
            <button onClick={() => setShowFeedbackRequest(true)} className="px-4 py-2 rounded-lg border border-ticano-red text-ticano-red text-sm hover:bg-red-50 dark:hover:bg-red-900/10 flex items-center gap-1.5">
              <Send size={14} /> Send Feedback Request
            </button>
          )}
      ...
      {showFeedbackRequest && (
        <SendFeedbackRequest
          open={showFeedbackRequest}
          onClose={() => setShowFeedbackRequest(false)}
          defaultInteractionType="complaint"
          defaultInteractionId={c.id}
          defaultInteractionNote={`Complaint ${c.ticket}`}
        />
      )}
...
function AssignModalContent({ isOpen, onClose, complaint, onAssigned }) {
  ...
  const handleAssign = () => {
    ...
    fn(complaint.id, { pmId: selectedPm.pmId, pmName: selectedPm.pmName }).then(() => {
      ...
      // Puts it on the assigned PM's dashboard/bell live, same gap as the
      // escalation path above.
      addNotification({
        userId: selectedPm.pmId,
        type: complaint.assignedPmId ? 'reassignment' : 'assignment',
        title: `${complaint.ticket} assigned to you`,
        body: `A complaint has been ${complaint.assignedPmId ? 're' : ''}assigned to you.`,
        tab: 'complaints',
      });
      onAssigned();
    });
...
function EscalateModalContent({ isOpen, onClose, complaint, currentUser, onEscalated }) {
  ...
  const { addNotification } = useNotifications();
  useEffect(() => { if (isOpen) setPriority(complaint?.priority || 'medium'); }, [isOpen, complaint]);
  // Chain of command: a PM can only escalate to their Service Manager; a
  // Service Manager forwarding it further can only send it to Director.
  // Nobody skips a level, and Director never sees a PM's escalation
  // directly, only ones a Service Manager has explicitly forwarded.
  const target = currentUser?.role === 'service_manager' ? 'director' : 'service_manager';
  const targetLabel = target === 'director' ? 'the Director' : 'your Service Manager';

  const handle = () => {
    if (!reason.trim()) return toast.error('Please provide an escalation reason');
    escalateComplaint(complaint.id, { reason, by: currentUser?.name || 'Staff', escalatedTo: target, priority })
      .then(() => {
        toast.success(`${complaint.ticket} escalated to ${targetLabel}`);
        // This is what makes the escalation actually show up on the
        // right dashboard/notification bell right away, on any device
        // not just the next time they refetch. Only the actual target
        // role is notified, a PM escalating to their Service Manager
        // must never also alert the Director prematurely.
        addNotification({
          audienceRole: target,
          type: 'escalation',
          title: `${complaint.ticket} escalated`,
          body: `${currentUser?.name || 'Staff'} escalated this complaint (${priority} priority): ${reason}`,
          tab: 'complaints',
        });
        onEscalated();
      })
      .catch((e) => toast.error(e.response?.data?.message || e.message || 'Could not escalate'));
  };
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Escalate to ${targetLabel}`}>
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
        This will route the case to {targetLabel}. Reason is required.
      </p>
      <label className="text-xs uppercase tracking-wide text-gray-500 mb-1 block">Priority</label>
      <select
        value={priority}
        onChange={(e) => setPriority(e.target.value)}
        className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm mb-3"
      >
        {COMPLAINT_PRIORITY.map((p) => (
          <option key={p.key} value={p.key}>{p.label}</option>
        ))}
      </select>
      <label className="text-xs uppercase tracking-wide text-gray-500 mb-1 block">Reason</label>
      <textarea
        value={reason}
...
function AuditTrailModal({ isOpen, onClose, complaint }) {
  ...
  useEffect(() => {
    if (!isOpen || !complaint) return;
    getAuditTrail({ complaintId: complaint.id }).then(({ data }) => setRows(data)).catch(() => setRows([]));
  }, [isOpen, complaint?.id]);
