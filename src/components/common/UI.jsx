...
import { Inbox, AlertTriangle, X, ChevronDown, ChevronRight, ArrowLeft, BarChart3, DatabaseZap } from 'lucide-react';
...
// EmptyState
// Flags a module that hasn't been migrated to the real database yet.
// Auth and Complaints are live; everything else still runs on sample data
// until it gets its own migration pass, this makes that visible instead
// of letting sample numbers pass as real ones.
export function NotConnectedBanner({ label = 'this section' }) {
  return (
    <div className="flex items-start gap-3 p-3.5 mb-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/40">
      <DatabaseZap size={16} className="text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
      <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
        <strong>Not yet connected to the live database</strong>, {label} is still showing sample data. Auth and Complaints are live; this module is next in line to migrate.
      </p>
    </div>
  );
}
...
// Small red unread-count pill used on tab buttons and hub launcher tiles
// disappears automatically once its parent passes count === 0 (i.e. once
// the relevant notifications have been marked read).
export function TabBadge({ count }) {
  if (!count) return null;
  return (
    <span className="ml-1.5 inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full bg-ticano-red text-white text-[10px] font-bold leading-none shrink-0 animate-fade-in">
      {count > 9 ? '9+' : count}
    </span>
  );
}
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-ticano-charcoal dark:text-white text-sm leading-snug flex items-center">{v.label}<TabBadge count={badges?.[v.id]} /></p>
...
export function AnalyticsBackBar({ view, onBack, backLabel = 'All analytics' }) {
