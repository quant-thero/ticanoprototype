...
import { APPLICATION_STATUSES } from '../../services/supabaseApi';
import { getJobApplications, getJobApplication, updateApplicationStatus, getSignedCvUrl } from '../../services/supabaseApi';
...
export default function JobApplicationsModule() {
  ...
  const load = () => {
    ...
    getJobApplications(statusFilter === 'All' ? {} : { status: statusFilter })
      ...
      .catch((err) => { console.error('[JobApplicationsModule]', err); toast.error('Could not load applications'); setLoading(false); });
  };
  ...
  const openCv = async (appId) => {
    try {
      const { data } = await getSignedCvUrl(appId);
      if (!data?.url) return toast.error('No CV was uploaded with this application');
  ...
  const downloadCv = async (appId) => {
    try {
      const { data } = await getSignedCvUrl(appId, true);
      if (!data?.url) return toast.error('No CV was uploaded with this application');
      const link = document.createElement('a');
      link.href = data.url;
      link.download = data.fileName || 'cv';
      document.body.appendChild(link); link.click(); document.body.removeChild(link);
    } catch {
      toast.error('Could not download the CV');
    }
  };
              ...
              {active?.id === a.id && (
                <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <p className="text-xs text-gray-400">CV file: <span className="font-mono text-gray-600 dark:text-gray-300">{a.cvFileName}</span></p>
                    <button onClick={() => openCv(a.id)} className="flex items-center gap-1 px-2.5 py-1 text-xs rounded-lg border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
                      <Eye size={11} /> View CV
                    </button>
                    <button onClick={() => downloadCv(a.id)} className="flex items-center gap-1 px-2.5 py-1 text-xs rounded-lg border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
