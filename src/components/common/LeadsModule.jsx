...
import { UserPlus, Send, Search, ArrowRight, Upload, FileSpreadsheet, Download, CheckCircle2, AlertTriangle, Copy, UserCheck } from 'lucide-react';
import * as XLSX from 'xlsx';
import toast from 'react-hot-toast';
import { Card, Badge, Modal, EmptyState, ExportButton } from './UI';
import { getLeads, createLead, importLeads, updateLeadStatus, convertLead, getActivePms, assignLead, autoAssignLeads, subscribeToTable, createFeedbackRequest } from '../../services/supabaseApi';
import {
  REFERRAL_SOURCES, LEAD_STATUSES, LEAD_STATUS_BADGE, BRANCHES, INTERESTED_PRODUCTS, normalizeWhatsAppNumber,
...
export default function LeadsModule({ branch }) {
  ...
  // Only Service Managers/Directors/Admins route leads to a PM, a PM
  // viewing their own book just sees who a lead is already assigned to.
  const canAssign = ['service_manager', 'director', 'admin'].includes(user?.role);
  const [pmOptions, setPmOptions] = useState([]);
  const [pickedPm, setPickedPm] = useState({}); // { [leadId]: pmId }
  ...
  useEffect(() => {
    load();
    if (canAssign) getActivePms().then(({ data }) => setPmOptions(data || [])).catch(() => {});
  }, []);

  // Live sync: a new potential client (self-registered or captured by
  // another branch/staff member) or a status/assignment change on an
  // existing one appears immediately, on every device, with no refresh.
  useEffect(() => {
    const unsubscribe = subscribeToTable('leads', {}, () => load());
    return unsubscribe;
  }, []);
  ...
  const handleAssignLead = async (leadId) => {
    try {
      const pmId = pickedPm[leadId] ? Number(pickedPm[leadId]) : null;
      await assignLead(leadId, pmId);
      const pmName = pmId ? pmOptions.find((p) => p.id === pmId)?.name : null;
      toast.success(pmName ? `Assigned to ${pmName}` : 'Assigned to the least-loaded PM');
      load();
  ...
  const handleAutoAssignLeads = async () => {
    try {
      ...
      toast.success(data.message || 'Potential clients auto-assigned');
      load();
  ...
  const FIELD_ALIASES = {
    ...
    company: 'companyName', 'company name': 'companyName', business: 'companyName', 'business name': 'companyName',
    industry: 'industry', sector: 'industry',
    'project scope': 'projectScope', scope: 'projectScope', 'project type': 'projectScope', 'application type': 'projectScope',
  ...
  const downloadTemplate = () => {
    const sample = [
      { 'Full Name': 'Kabo Otsile', 'Phone Number': '+26771000000', 'Company Name': 'Kabo Traders (Pty) Ltd', 'Industry': 'Retail', 'Branch': 'Gaborone', 'Interested Product': 'PO Financing', 'Project Scope': 'Supplier financing for a government retail tender', 'Referral Source': 'Walk-in', 'Notes': 'Asked about supplier financing' },
      { 'Full Name': 'Neo Bareki', 'Phone Number': '+26772000000', 'Company Name': '', 'Industry': 'Construction', 'Branch': 'Francistown', 'Interested Product': 'Invoice Discounting', 'Project Scope': '', 'Referral Source': 'Facebook', 'Notes': '' },
    ];
    const ws = XLSX.utils.json_to_sheet(sample);
    ws['!cols'] = [{ wch: 20 }, { wch: 16 }, { wch: 22 }, { wch: 14 }, { wch: 14 }, { wch: 20 }, { wch: 26 }, { wch: 18 }, { wch: 30 }];
  ...
  const submit = () => {
    ...
    createLead(payload).then(() => {
      ...
      load();
    }).catch((err) => toast.error(err?.message || 'Could not save this potential client'));
  };
  ...
  const sendLink = (l) => {
    const digits = normalizeWhatsAppNumber(l.phone);
    if (!digits) return toast.error(`No valid WhatsApp number for ${l.name}`);
    createFeedbackRequest({
      interactionType: 'follow_up', clientId: null, clientName: l.name, clientPhone: l.phone,
      interactionNote: 'Lead follow-up',
    }).then(({ data }) => {
      const message = `Hi ${l.name}, thank you for your interest in Ticano! We'd love to hear your thoughts: ${data.link}`;
      window.open(`https://wa.me/${digits}?text=${encodeURIComponent(message)}`, '_blank');
      toast.success(`Feedback request sent to ${l.name} via WhatsApp`);
    }).catch((err) => toast.error(err?.message || 'Could not generate feedback link'));
  };
    ...
    <Card
      ...
      actions={
        <div className="flex gap-2">
          ...
          {canAssign && (
            <button onClick={handleAutoAssignLeads} className="flex items-center gap-2 px-4 py-2 bg-ticano-charcoal text-white rounded-lg text-sm font-medium hover:bg-black">
          ...
          <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-4 py-2 bg-ticano-red text-white rounded-lg text-sm font-medium hover:bg-ticano-red-dark">
            <UserPlus size={16} /> New Potential Client
          </button>
        </div>
      }
    >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {[...['Name', 'Phone', 'Company', 'Industry', 'Branch', 'Scope', 'Referral', 'Product', 'Added', 'Status'], ...(canAssign ? ['Assigned PM'] : []), 'Actions'].map((h) => (
            ...
            <tbody>
              {filtered.map((l) => (
                <tr key={l.id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <td className="py-3 px-2 text-gray-500">{l.companyName || '-'}</td>
                  <td className="py-3 px-2 text-gray-500">{l.industry || '-'}</td>
                  <td className="py-3 px-2 text-gray-500">{l.branch}</td>
                  <td className="py-3 px-2 text-gray-500 max-w-[160px] truncate" title={l.projectScope || ''}>{l.projectScope || '-'}</td>
                  <td className="py-3 px-2 text-gray-500">{l.referralSource}</td>
                  <td className="py-3 px-2 text-gray-500">{l.product}</td>
                  <td className="py-3 px-2 text-gray-400" title="Application date">{formatDate(l.applicationDate || l.addedAt)}</td>
                  ...
                  {canAssign && (
                    <td className="py-3 px-2">
                      {l.assignedPmName ? (
                        <span className="text-xs px-2 py-1 rounded-full bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-300">{l.assignedPmName}</span>
                      ) : (
                        <div className="flex items-center gap-1.5">
                          <select
                            value={pickedPm[l.id] || ''}
                            onChange={(e) => setPickedPm((p) => ({ ...p, [l.id]: e.target.value }))}
                            className="text-xs border border-gray-200 dark:border-gray-600 rounded-md px-1.5 py-1 bg-white dark:bg-gray-800 dark:text-white"
                          >
                            <option value="">Auto</option>
                            {pmOptions.map((pm) => <option key={pm.id} value={pm.id}>{pm.name}</option>)}
                          </select>
                          <button onClick={() => handleAssignLead(l.id)} title="Assign to PM" className="text-xs px-2 py-1 rounded-md bg-ticano-red text-white hover:bg-ticano-red-dark">
                            Assign
                          </button>
                        </div>
                      )}
                  ...
                  <td className="py-3 px-2">
                    <div className="flex items-center gap-2">
                      <button onClick={() => sendLink(l)} title="Send feedback request" className="p-1.5 rounded-md text-ticano-red hover:bg-ticano-red-light">
                        <Send size={14} />
                      </button>
                      {l.status !== 'Converted' && (
        ...
        {!importResult ? (
          <div className="space-y-4">
            <p className="text-sm text-gray-600 dark:text-gray-300">
              Upload an Excel (.xlsx.xls) or CSV file with the same details as the New Potential Client form. Required columns: <b>Full Name</b> and <b>Phone Number</b>. Optional: Branch, Interested Product, Referral Source, Notes.
            </p>
                ...
                <div className="max-h-52 overflow-auto border border-gray-100 dark:border-gray-700 rounded-lg">
                  <table className="w-full text-xs">
                    <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0">
                      <tr>
                        {['Name', 'Phone', 'Company', 'Industry', 'Branch', 'Product', 'Referral', 'Notes'].map((h) => (
                    ...
                    <tbody>
                      {parsedRows.slice(0, 50).map((r, i) => (
                        <tr key={i} className="border-t border-gray-100 dark:border-gray-800">
                          ...
                          <td className="py-1.5 px-2 text-gray-500">{r.companyName || '-'}</td>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button onClick={submit} className="px-4 py-2 text-sm rounded-lg bg-ticano-red text-white hover:bg-ticano-red-dark">Save Potential Client</button>
          </>
        }
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Company Name"><input value={form.companyName} onChange={(e) => setForm({ ...form, companyName: e.target.value })} placeholder="e.g. Kabo Traders (Pty) Ltd" className={`${sel} w-full`} /></Field>
          <Field label="Industry"><input value={form.industry} onChange={(e) => setForm({ ...form, industry: e.target.value })} placeholder="e.g. Construction, Retail…" className={`${sel} w-full`} /></Field>
          <Field label="Branch">
          ...
          <div className="sm:col-span-2">
            <Field label="Project Scope / Type"><textarea rows={2} value={form.projectScope} onChange={(e) => setForm({ ...form, projectScope: e.target.value })} placeholder="What are they looking to finance, and at what stage?" className={`${sel} w-full resize-none`} /></Field>
          </div>
          <div className="sm:col-span-2">
...
function blankForm(branch) {
  return { name: '', phone: '', branch: branch || 'Gaborone', product: 'General Enquiry', companyName: '', industry: '', projectScope: '', referralSource: '', referralOther: '', notes: '' };
}
