...
import { getPublishedQuestionnaires, submitQuestionnaireResponse, clientIdFor } from '../../services/api';
...
export default function ClientQuestionnaires() {
  ...
  const submit = async () => {
    ...
    await submitQuestionnaireResponse(active.id, {
      clientId: clientIdFor(user?.userId || user?.id || 1),
  ...
  return (
    <div className="space-y-3">
      {visible.map((q) => (
        <div key={q.id} className="bg-gradient-to-br from-ticano-red/5 to-transparent border border-ticano-red/20 rounded-2xl p-4">
