...
import { getBirthdayPrefs, saveBirthdayPrefs, simulateBirthdaySend } from '../../services/api';
...
export default function BirthdayPreferences() {
  ...
  useEffect(() => {
    getBirthdayPrefs(user?.id || 1).then(({ data }) => { setPrefs(data); setLoading(false); });
  }, []);

  const save = async () => {
    setSaving(true);
    await saveBirthdayPrefs(user?.id || 1, prefs);
    toast.success('Birthday preferences saved');
    setSaving(false);
  };

  const simulate = async () => {
    const { data } = await simulateBirthdaySend(user?.id || 1);
    if (data.sent) {
      setSimulated(data);
      toast.success(`Birthday message preview sent via ${data.channel}!`);
    } else {
      toast.error('Enable birthday messages first');
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 p-4 bg-gradient-to-r from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20 rounded-2xl border border-pink-200 dark:border-pink-800">
