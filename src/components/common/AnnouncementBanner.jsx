...
export default function AnnouncementBanner() {
  ...
  useEffect(() => {
    getAnnouncements({ role: user?.role }).then(({ data }) => setAnnouncements(data));
  }, [user?.role]);
        ...
        <div key={ann.id} className={`flex gap-3 px-4 py-3 rounded-xl border text-sm transition-all duration-300 ${priorityStyle[ann.priority] || priorityStyle.info}`}
          ...
          <div className="flex items-start gap-2 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] opacity-50 mt-1">From {ann.author} · {new Date(ann.createdAt).toLocaleDateString()}</p>
            </div>
          </div>
