...
import { generateReport } from '../../services/supabaseApi';
...
const downloadReportPdf = async (result) => {
  const { default: jsPDF } = await import('jspdf');
  const { default: autoTable } = await import('jspdf-autotable');
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });

  doc.setFontSize(16);
  doc.setTextColor(206, 49, 60); // ticano red
  doc.text('Ticano Group', 14, 18);
  doc.setFontSize(12);
  doc.setTextColor(55, 52, 53); // ticano charcoal
  doc.text(result.type, 14, 26);
  doc.setFontSize(9);
  doc.setTextColor(130, 130, 130);
  doc.text(`Generated ${new Date(result.generatedAt).toLocaleString()} · ${result.summary.period} · ${result.summary.branch}`, 14, 32);

  autoTable(doc, {
    startY: 40,
    head: [['Metric', 'Value']],
    body: [
      ['Total Complaints', String(result.summary.totalComplaints)],
      ['Resolved', String(result.summary.resolved)],
      ['Escalated', String(result.summary.escalated)],
      ['Avg CSAT', `${result.summary.avgCsat}/5`],
      ['SLA Breaches', String(result.summary.slaBreaches)],
      ['Top Issue', result.summary.topIssue || 'N/A'], ...(result.summary.totalCustomers !== undefined ? [
        ['Total Customers', String(result.summary.totalCustomers)],
        ['New This Month', String(result.summary.newThisMonth)],
        ['Referral Conversion Rate', `${result.summary.referralConversionRate}%`],
      ] : []),
    ],
    theme: 'striped',
    headStyles: { fillColor: [206, 49, 60] },
  });

  doc.save(`${result.type.replace(/\s+/g, '_')}_${result.reportId}.pdf`);
};
      ...
      {result && (
        <div className="bg-white dark:bg-ticano-dark-card border border-gray-100 dark:border-gray-700 rounded-xl p-5 animate-scale-in">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => downloadReportPdf(result).catch(() => toast.error('Could not generate PDF'))}
