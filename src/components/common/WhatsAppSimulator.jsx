...
import { getWaTemplates, getClientDirectory } from '../../services/api';
import { useAuth } from '../../context/AuthContext';
import toast from 'react-hot-toast';

const formatTime = () => new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
const REVIEW_LINK = 'https://ticanogroup.co.bw/review';

export default function WhatsAppSimulator({ clientName = 'Stacey Nthoi', clientPhone = '+267 71 234 567', branch }) {
  const { user } = useAuth();
  ...
  const autoVarsFor = (tpl, client, ticket) => {
    ...
    (tpl?.variables || []).forEach((v) => {
      switch (v) {
        ...
        case 'Link': out[v] = REVIEW_LINK; break;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      ...
      <div className="bg-white dark:bg-ticano-dark-card rounded-2xl border border-gray-100 dark:border-gray-700 p-5 shadow-sm">
        ...
        <div className="flex items-center justify-between text-xs text-gray-400 mb-3">
          <span>To: {activeName} · {activePhone}</span>
        </div>
