  ...
  if (withWordmark || withTagline) {
    const svg = (
      <svg
        ...
        className={`${spin} ${withTagline ? '' : className}`}
        style={{ display: 'block' }}
        aria-label="Ticano"
        role="img"
      >
    ...
    if (!withTagline) return svg;
    return (
      <div className={`flex flex-col items-center ${className}`}>
        {svg}
        <p className={`text-[10px] leading-tight tracking-wide whitespace-nowrap mt-0.5 text-center ${taglineClassName || 'text-gray-400'}`}>
          Purchase Order Financing Specialists
        </p>
      </div>
    );
  }
