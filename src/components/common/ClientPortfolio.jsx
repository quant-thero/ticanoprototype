...
const FREQ_OPTIONS = [
  { value: '', label: 'Any frequency' }, ...[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => ({ value: String(n), label: `Assisted ${n} time${n > 1 ? 's' : ''}` })),
...
export default function ClientPortfolio({ mode = 'pm' }) {
  ...
  const load = () => {
    ...
    const scopeFilters = mode === 'pm' ? { pmId: user?.id } : { orgWide: true };
    getPortfolioClients({ ...filters, ...scopeFilters })
      .then(({ data }) => { setClients(data); setLoading(false); })
      .catch((err) => { console.error('[ClientPortfolio] failed to load clients:', err); toast.error('Could not load client portfolio'); setLoading(false); });
    getPortfolioInsights(scopeFilters).then(({ data }) => setInsights(data)).catch(() => {});
  };
  ...
  const openProfile = (id) => getPortfolioClient(id).then(({ data }) => setProfile(data)).catch(() => toast.error('Could not load client profile'));
  const refreshProfile = () => { if (profile) getPortfolioClient(profile.id).then(({ data }) => setProfile(data)).catch(() => {}); };
        ...
        actions={
          <div className="flex gap-2 flex-wrap">
            ...
            <ExportButton rows={clients.map((c) => ({
              ...
              'Last Contact': c.lastContactDate || '', 'Next Follow-up': c.nextFollowUpDate || '', ...(editable ? {} : { 'Portfolio Manager': c.pmName }),
