...
export default function ComplaintForm({ onSubmit, clientType = 'existing', defaultBranch = 'Gaborone' }) {
  ...
  // Tracks a finalization (blob -> base64) that's still in progress,
  // regardless of which button triggered it, a standalone "Stop
  // Recording" click and the "Submit" click's own stop-if-recording both
  // go through this, so whichever one actually started the finalization,
  // BOTH wait on the same promise instead of racing each other.
  const finalizingRef = useRef(null);

  const startRecording = async () => {
    console.log('[VOICE] startRecording called');
    setRecError('');
    if (!navigator.mediaDevices || typeof MediaRecorder === 'undefined') {
      console.error('[VOICE] MediaRecorder not supported in this browser');
  ...
  // Returns a Promise resolving to the base64 audio data once it's
  // genuinely ready, MediaRecorder.stop() -> onstop -> FileReader is an
  // async chain, so callers that need the actual data (handleSubmit)
  // must await this rather than reading the audioData state variable
  // immediately afterward, which would still hold its stale pre-stop
  // value (null, on a first recording) since React state updates don't
  // happen synchronously mid-function.
  const stopRecordingAndGetData = () => {
    console.log('[VOICE] stopRecordingAndGetData called. recorder state:', mediaRecRef.current?.state, 'already finalizing:', Boolean(finalizingRef.current));
    // Already finalizing (e.g. the standalone Stop button was clicked and
    // its FileReader hasn't resolved yet), return that SAME promise
    // rather than starting a second, redundant finalization.
    ...
    const promise = new Promise((resolve) => {
      ...
      if (!rec || rec.state === 'inactive') {
        console.log('[VOICE] recorder missing or already inactive, resolving with existing audioData, length:', audioData?.length ?? 'null');
        setRecording(false);
        resolve(audioData); // nothing was actively recording, whatever we already have
        return;
      }
      const originalOnStop = rec.onstop;
      rec.onstop = (e) => {
        console.log('[VOICE] MediaRecorder onstop fired. chunks:', chunksRef.current.length, 'mimeType:', rec.mimeType);
        originalOnStop?.(e); // still runs the existing blob/preview-URL logic
        const blob = new Blob(chunksRef.current, { type: rec.mimeType || 'audio/webm' });
        console.log('[VOICE] blob created, size:', blob.size, 'bytes, type:', blob.type);
        const reader = new FileReader();
        reader.onloadend = () => {
          console.log('[VOICE] FileReader finished, base64 length:', reader.result?.length ?? 'null');
          finalizingRef.current = null;
          resolve(reader.result);
        };
        reader.onerror = () => {
          console.error('[VOICE] FileReader FAILED:', reader.error);
          finalizingRef.current = null;
          resolve(null);
        };
        reader.readAsDataURL(blob);
      };
      try { rec.stop(); } catch (err) { console.error('[VOICE] rec.stop() threw:', err); finalizingRef.current = null; resolve(audioData); }
      setRecording(false);
    });
  ...
  // Kept for the standalone "Stop Recording" button, where nothing needs
  // to await the result immediately afterward.
  const stopRecording = () => { stopRecordingAndGetData(); };
  ...
  const handleSubmit = async () => {
    ...
    if (recording || finalizingRef.current) {
      // Covers both: still actively recording right now, AND a prior
      // separate "Stop Recording" click whose blob-to-base64 conversion
      // hasn't finished yet, either way, wait for the real result
      // instead of using whatever audioData happens to hold this instant.
      finalAudioData = await stopRecordingAndGetData();
    }
    console.log('[VOICE] handleSubmit proceeding. finalAudioData present:', Boolean(finalAudioData), 'length:', finalAudioData?.length ?? 'null');
    if (!form.description.trim() && !finalAudioData) return toast.error('Please describe your complaint, or record a voice note');
    setSubmitting(true);
    try {
      await onSubmit?.({ ...form, clientType, branch: defaultBranch, voiceNote: finalAudioData, voiceNoteDuration: finalAudioData ? elapsed : null });
      baseTextRef.current = '';
      reRecord();
      setForm({
        journeyStage: 'before_applying',
