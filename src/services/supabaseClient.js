...
export async function testSupabaseConnection() {
  if (!isSupabaseConfigured) {
    console.warn('⚠️ [supabase] Not configured — fill in .env.local and restart. Skipping connection test.');
    return;
  }
  ...
  if (!dbErr) {
    ...
    console.warn('⚠️ [supabase] Connected, but the schema isn\'t loaded yet (no "branches" table). Run supabase/schema.sql in the SQL editor.');
