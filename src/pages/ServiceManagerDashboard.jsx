...
import { Users, AlertTriangle, TrendingUp, Activity, UserCheck, Send, ShieldAlert, ArrowUpRightSquare, UserPlus2, Lightbulb, Clock, BookOpen } from 'lucide-react';
import { LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend, PieChart, Pie, Cell } from 'recharts';
import Navbar from '../components/common/Navbar';
import { StatCard, Badge, LoadingSpinner, Card, EmptyState } from '../components/common/UI';
...
export default function ServiceManagerDashboard() {
  ...
  return (
    <div className="min-h-screen bg-ticano-bg-light dark:bg-ticano-dark-bg">
      <Navbar title={`Service Manager — ${user?.branch}`} />
      <div className="max-w-7xl mx-auto px-4 py-6">

        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          <h1 className="text-xl font-bold text-ticano-charcoal dark:text-white">{user?.branch} Branch</h1>
            ...
            {escalations.length > 0 && (
              <Card title="Active escalations" subtitle="Complaints requiring management decisions" className="mt-4">
                <div className="space-y-2">
                  {escalations.map((c) => (
                    <div key={c.id} className="flex items-center justify-between p-3 rounded-lg bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-700/30">
                      <div>
                        <p className="text-sm font-semibold text-orange-900 dark:text-orange-200">{c.ticket} · {c.customerName}</p>
                        <p className="text-xs text-gray-600 dark:text-gray-300">{c.category} · {c.escalation?.reason}</p>
                      </div>
                      <span className="text-xs text-orange-700 dark:text-orange-300">{formatDateTime(c.escalation?.at)}</span>
                    </div>
                  ))}
                </div>
              </Card>
        ...
        {/* ---------- ACTIVE CLIENTS ---------- */}
        {activeTab === 'Active Clients' && activeClients && (
        ...
        {/* ---------- LEADS ---------- */}
        {activeTab === 'Leads' && <LeadsModule branch={user?.branch} />}
        ...
        {activeTab === 'Staff Performance' && (
          <Card title="Staff Performance" subtitle={`${user?.branch} branch`}>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-700">
        ...
        {/* ---------- ANALYTICS ---------- */}
        {activeTab === 'Analytics' && complaintAnalytics && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {activeTab === 'Appointments' && (
          <div className="bg-white dark:bg-ticano-dark-card rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            <h3 className="font-bold text-ticano-charcoal dark:text-white text-lg mb-1">Branch Appointments</h3>
            <p className="text-sm text-gray-500 mb-4">Manage all scheduled appointments and callbacks for {user?.branch}</p>
            <AppointmentModule branch={user?.branch} canCreate={true} />
          </div>
        )}

        {activeTab === 'Charts' && (
          <div className="bg-white dark:bg-ticano-dark-card rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            <h3 className="font-bold text-ticano-charcoal dark:text-white text-lg mb-1">Branch Analytics</h3>
            <p className="text-sm text-gray-500 mb-5">Interactive charts for your branch</p>
            <AdvancedCharts />
          </div>
        )}
        ...
        {activeTab === 'Reports' && (
          <div className="bg-white dark:bg-ticano-dark-card rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <p className="text-sm text-gray-500 mb-5">Generate and download branch performance reports</p>
            <PDFReportGenerator availableTypes={['complaints_summary','branch_performance','sla_breach','staff_performance','client_satisfaction']} />
          </div>
        )}
