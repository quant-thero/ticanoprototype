...
import { getFeedbackForm, submitFeedback } from '../services/api';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!data.valid) setError('This feedback link has expired or is invalid.');
        else setForm(data);
      } catch {
    ...
    <div className="min-h-screen bg-ticano-bg-light flex items-center justify-center px-4 py-8">
      <div className="bg-white rounded-2xl shadow-xl p-6 max-w-md w-full">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-xl font-bold text-ticano-charcoal">Rate Your Experience</h1>
          <p className="text-gray-500 text-sm mt-1">
            {form?.branch} Branch · {form?.serviceType || 'Service'}
          </p>
        </div>
