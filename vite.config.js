  ...
  return {
    ...
    configureServer() {
      ...
      if (!url || !key || url.includes('YOUR-PROJECT-REF')) {
        line(`🐤 ${tag} Not configured yet — fill in .env.local. The app will run on mock data for now.`)
        return
      }
      ...
      fetch(`${url}/rest/v1/branches?select=id&limit=1`, {
        ...
        .then((res) => {
          if (res.ok) {
            line(`🎉 ${tag} Connected! URL, key, and database are all reachable. You're good to go ✨`)
            ...
            line(`😿 ${tag} Reached Supabase but got HTTP ${res.status}. Something's off.`)
          }
        })
        .catch((e) => line(`💥 ${tag} Connection failed — check VITE_SUPABASE_URL / your network. (${e.message})`))
    },
  }
}
